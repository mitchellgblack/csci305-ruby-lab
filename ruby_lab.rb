...
def cleanup_title line
	...
	# converting title to lowercase so following regexes will match
	line.downcase!
	...
	# checking if the title has non-English characters
...
def mcw word
	...
	if $bigrams.member? word           # checking if the input word appears in the songlist
		max_val = 0
		max_key = ""
		$bigrams[word].each do |key, val|
			if val > max_val               # finding the key with the highest value
				max_key = key
				max_val = val
			end
		end
